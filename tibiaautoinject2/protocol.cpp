...
std::string NetworkMessage::GetHex(int len/*=0*/){
	if(!len)
		len = msgSize-readPos;
	if(len > (NETWORKMESSAGE_MAXSIZE - readPos-1)){
		AfxMessageBox("TibiaautoInject2: NetworkMessage GetHex trying to read past buffer");
		return std::string();
	}
	char ret[1000];
	char *reti = ret;
...
void Protocol::parsePacketOut(NetworkMessage &msg){
	try{
		...
		CString tmp = CString();
		char shortmsg[1000] = "";
		memcpy(shortmsg,msg.msgBuf,1000);
		switch(recvbyte)
		{
			case 0x89: //parseTextWindow(msg);
			...
			case 0x87: // close container
				...
				tmp.Format(" %d",msg.GetByte());//ContNr
				description+=tmp;
				break;
			case 0x88: //"up-arrow" - container
			...
			case 0xA1: // attack
				...
				tmp.Format(" It:%d",msg.GetInt());//PacketIterator
				description+=tmp;
				break;
			case 0xA2: //follow
				description = "Follow";
			...
			case 0xF8: //Accept buy/sell offer
				...
				// F8 05 AE 1B 4F 00 00 05 01
			default:
				description.Format("Unknown Packet %x",recvbyte);
				break;
		}
		description += CString(" ")+msg.GetHex().c_str();
		sendTAMessage(description.GetBuffer(2048));
