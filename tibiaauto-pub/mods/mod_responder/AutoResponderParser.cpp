...
int CAutoResponderParser::processNodeBoolCondition(DOMNode *node, CAutoResponderParserContext *context)
{
	...
	for (nodeNr = 0; nodeNr < node->getChildNodes()->getLength() && !autoResponderStop; nodeNr++)
	{
		...
		if (!strcmp(nodeName, "and"))
			processResult = processResult && processNodeBoolAnd(childNode, context), hit = 1;
		if (!strcmp(nodeName, "channel"))
			processResult = processResult && processNodeBoolChannel(childNode, context), hit = 1;
		if (!strcmp(nodeName, "or"))
			processResult = processResult && processNodeBoolOr(childNode, context), hit = 1;
		if (!strcmp(nodeName, "playername"))
			processResult = processResult && processNodeBoolPlayername(childNode, context), hit = 1;
		if (!strcmp(nodeName, "message"))
			processResult = processResult && processNodeBoolMessage(childNode, context), hit = 1;
		if (!strcmp(nodeName, "testvariable"))
			processResult = processResult && processNodeBoolTestvariable(childNode, context), hit = 1;
		if (!strcmp(nodeName, "not"))
			processResult = processResult && processNodeBoolNot(childNode, context), hit = 1;
...
void CAutoResponderParser::processAction(DOMNode *node, CAutoResponderParserContext *context)
{
	...
	if (!strcmp(nodeName, "dance"))
		processNodeDance(node, context), hit = 1;
	if (!strcmp(nodeName, "if"))
		processNodeIf(node, context), hit = 1;
	if (!strcmp(nodeName, "ignoreplayer"))
		processNodeIgnoreplayer(node, context), hit = 1;
	if (!strcmp(nodeName, "random"))
		processNodeRandom(node, context), hit = 1;
	if (!strcmp(nodeName, "repeat"))
		processNodeRepeat(node, context), hit = 1;
	if (!strcmp(nodeName, "sequence"))
		processNodeSequence(node, context), hit = 1;
	if (!strcmp(nodeName, "say"))
		processNodeSay(node, context), hit = 1;
	if (!strcmp(nodeName, "setvariable"))
		processNodeSetvariable(node, context), hit = 1;
	if (!strcmp(nodeName, "sleep"))
		processNodeSleep(node, context), hit = 1;
