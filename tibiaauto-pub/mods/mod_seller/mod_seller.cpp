...
int CMod_SellerApp::validateConfig(int showAlerts)
{
	...
	{
		if (showAlerts)
			AfxMessageBox("\"Sell when capacity is less than:\" amount must not be 0(zero)!!");
		return 0;
	}
	for (int i = 0; i < MAX_SELLERS; i++)
	{
...
void CMod_SellerApp::loadConfigParam(const char *paramName, char *paramValue)
{
	...
	if (!strcmp(paramName, "SellOnCap"))
		m_configData->sellOnCap = atoi(paramValue);
	if (!strcmp(paramName, "SellOnSpace"))
		m_configData->sellOnSpace = atoi(paramValue);
	if (!strcmp(paramName, "SellWhen"))
		m_configData->sellWhen = atoi(paramValue);
	if (!strcmp(paramName, "SuggestBanker"))
		m_configData->suggestBanker = atoi(paramValue);
// Seller 1
	if (!strcmp(paramName, "Seller1/Name"))
		sprintf(m_configData->sellerList[0].sellerName, "%s", paramValue);
	if (!strcmp(paramName, "Seller1/Pos"))
		sscanf(paramValue, "(%d,%d,%d)", &(m_configData->sellerList[0].sellerX), &(m_configData->sellerList[0].sellerY), &(m_configData->sellerList[0].sellerZ));
...
// Seller 2
	if (!strcmp(paramName, "Seller2/Name"))
		sprintf(m_configData->sellerList[1].sellerName, "%s", paramValue);
	if (!strcmp(paramName, "Seller2/Pos"))
		sscanf(paramValue, "(%d,%d,%d)", &(m_configData->sellerList[1].sellerX), &(m_configData->sellerList[1].sellerY), &(m_configData->sellerList[1].sellerZ));
...
// Seller 3
	if (!strcmp(paramName, "Seller3/Name"))
		sprintf(m_configData->sellerList[2].sellerName, "%s", paramValue);
	if (!strcmp(paramName, "Seller3/Pos"))
		sscanf(paramValue, "(%d,%d,%d)", &(m_configData->sellerList[2].sellerX), &(m_configData->sellerList[2].sellerY), &(m_configData->sellerList[2].sellerZ));
...
// Seller 4
	if (!strcmp(paramName, "Seller4/Name"))
		sprintf(m_configData->sellerList[3].sellerName, "%s", paramValue);
	if (!strcmp(paramName, "Seller4/Pos"))
		sscanf(paramValue, "(%d,%d,%d)", &(m_configData->sellerList[3].sellerX), &(m_configData->sellerList[3].sellerY), &(m_configData->sellerList[3].sellerZ));
...
char *CMod_SellerApp::saveConfigParam(const char *paramName)
{
	...
	buf[0] = '\0';
	if (!strcmp(paramName, "StopBySeller"))
		sprintf(buf, "%d", m_configData->stopBySeller);
	if (!strcmp(paramName, "ModPriority"))
		sprintf(buf, "%s", m_configData->modPriorityStr);
	if (!strcmp(paramName, "SellOnCap"))
		sprintf(buf, "%d", m_configData->sellOnCap);
	if (!strcmp(paramName, "SellOnSpace"))
		sprintf(buf, "%d", m_configData->sellOnSpace);
	if (!strcmp(paramName, "SellWhen"))
		sprintf(buf, "%d", m_configData->sellWhen);
	if (!strcmp(paramName, "SuggestBanker"))
		sprintf(buf, "%d", m_configData->suggestBanker);
// Seller 1
	if (!strcmp(paramName, "Seller1/Name"))
		sprintf(buf, "%s", m_configData->sellerList[0].sellerName);
	if (!strcmp(paramName, "Seller1/Pos"))
		sprintf(buf, "(%d,%d,%d)", m_configData->sellerList[0].sellerX, m_configData->sellerList[0].sellerY, m_configData->sellerList[0].sellerZ);
	if (!strcmp(paramName, "Seller1/SaleItems/Name"))
	{
		if (currentPos < MAX_SELLER_ITEMS)
...
// Seller 2
	if (!strcmp(paramName, "Seller2/Name"))
		sprintf(buf, "%s", m_configData->sellerList[1].sellerName);
	if (!strcmp(paramName, "Seller2/Pos"))
		sprintf(buf, "(%d,%d,%d)", m_configData->sellerList[1].sellerX, m_configData->sellerList[1].sellerY, m_configData->sellerList[1].sellerZ);
	if (!strcmp(paramName, "Seller2/SaleItems/Name"))
	{
		if (currentPos < MAX_SELLER_ITEMS)
...
// Seller 3
	if (!strcmp(paramName, "Seller3/Name"))
		sprintf(buf, "%s", m_configData->sellerList[2].sellerName);
	if (!strcmp(paramName, "Seller3/Pos"))
		sprintf(buf, "(%d,%d,%d)", m_configData->sellerList[2].sellerX, m_configData->sellerList[2].sellerY, m_configData->sellerList[2].sellerZ);
	if (!strcmp(paramName, "Seller3/SaleItems/Name"))
	{
		if (currentPos < MAX_SELLER_ITEMS)
...
// Seller 4
	if (!strcmp(paramName, "Seller4/Name"))
		sprintf(buf, "%s", m_configData->sellerList[3].sellerName);
	if (!strcmp(paramName, "Seller4/Pos"))
		sprintf(buf, "(%d,%d,%d)", m_configData->sellerList[3].sellerX, m_configData->sellerList[3].sellerY, m_configData->sellerList[3].sellerZ);
	if (!strcmp(paramName, "Seller4/SaleItems/Name"))
	{
		if (currentPos < MAX_SELLER_ITEMS)
