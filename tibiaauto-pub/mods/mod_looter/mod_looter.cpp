...
DWORD WINAPI toolThreadProc(LPVOID lpParam)
{
	...
	while (!toolThreadShouldStop)
	{
		...
		if (carriedContainers != 0)
		{
			...
			int extraSleep = 0;
			if (config->m_mode1 == 2 && CModuleUtil::lootItemsToSpecifiedContainers(0, &acceptedItems, carriedContainers) != -1)
				extraSleep = 1;
			if (config->m_mode2 == 2 && CModuleUtil::lootItemsToSpecifiedContainers(1, &acceptedItems, carriedContainers) != -1)
				extraSleep = 1;
			if (config->m_mode3 == 2 && CModuleUtil::lootItemsToSpecifiedContainers(2, &acceptedItems, carriedContainers) != -1)
				extraSleep = 1;
			if (config->m_mode4 == 2 && CModuleUtil::lootItemsToSpecifiedContainers(3, &acceptedItems, carriedContainers) != -1)
				extraSleep = 1;
			if (config->m_mode5 == 2 && CModuleUtil::lootItemsToSpecifiedContainers(4, &acceptedItems, carriedContainers) != -1)
				extraSleep = 1;
			if (config->m_mode6 == 2 && CModuleUtil::lootItemsToSpecifiedContainers(5, &acceptedItems, carriedContainers) != -1)
				extraSleep = 1;
			if (config->m_mode7 == 2 && CModuleUtil::lootItemsToSpecifiedContainers(6, &acceptedItems, carriedContainers) != -1)
				extraSleep = 1;
			if (config->m_mode8 == 2 && CModuleUtil::lootItemsToSpecifiedContainers(7, &acceptedItems, carriedContainers) != -1)
				extraSleep = 1;
			if (config->m_mode9 == 2 && CModuleUtil::lootItemsToSpecifiedContainers(8, &acceptedItems, carriedContainers) != -1)
				extraSleep = 1;
			if (config->m_mode10 == 2 && CModuleUtil::lootItemsToSpecifiedContainers(9, &acceptedItems, carriedContainers) != -1)
				extraSleep = 1;
			if (extraSleep)
...
void CMod_looterApp::loadConfigParam(char *paramName, char *paramValue)
{
	...
	if (!strcmp(paramName, "mode/cont2"))
		m_configData->m_mode2 = atoi(paramValue);
	if (!strcmp(paramName, "mode/cont3"))
		m_configData->m_mode3 = atoi(paramValue);
	if (!strcmp(paramName, "mode/cont4"))
		m_configData->m_mode4 = atoi(paramValue);
	if (!strcmp(paramName, "mode/cont5"))
		m_configData->m_mode5 = atoi(paramValue);

	if (!strcmp(paramName, "mode/cont6"))
		m_configData->m_mode6 = atoi(paramValue);
	if (!strcmp(paramName, "mode/cont7"))
		m_configData->m_mode7 = atoi(paramValue);
	if (!strcmp(paramName, "mode/cont8"))
		m_configData->m_mode8 = atoi(paramValue);
	if (!strcmp(paramName, "mode/cont9"))
		m_configData->m_mode9 = atoi(paramValue);
	if (!strcmp(paramName, "mode/cont10"))
		m_configData->m_mode10 = atoi(paramValue);

	if (!strcmp(paramName, "loot/worms"))
		m_configData->m_lootWorms = atoi(paramValue);
	if (!strcmp(paramName, "loot/inDepot"))
		m_configData->m_lootInDepot = atoi(paramValue);
	if (!strcmp(paramName, "loot/gp"))
		m_configData->m_lootGp = atoi(paramValue);
	if (!strcmp(paramName, "loot/food"))
		m_configData->m_lootFood = atoi(paramValue);
	if (!strcmp(paramName, "loot/custom"))
		m_configData->m_lootCustom = atoi(paramValue);
	if (!strcmp(paramName, "other/autoOpen"))
		m_configData->m_autoOpen = atoi(paramValue);
	if (!strcmp(paramName, "loot/eatFromCorpse"))
...
char *CMod_looterApp::saveConfigParam(char *paramName)
{
	...
	if (!strcmp(paramName, "mode/cont1"))
		sprintf(buf, "%d", m_configData->m_mode1);
	if (!strcmp(paramName, "mode/cont2"))
		sprintf(buf, "%d", m_configData->m_mode2);
	if (!strcmp(paramName, "mode/cont3"))
		sprintf(buf, "%d", m_configData->m_mode3);
	if (!strcmp(paramName, "mode/cont4"))
		sprintf(buf, "%d", m_configData->m_mode4);
	if (!strcmp(paramName, "mode/cont5"))
		sprintf(buf, "%d", m_configData->m_mode5);

	if (!strcmp(paramName, "mode/cont6"))
		sprintf(buf, "%d", m_configData->m_mode6);
	if (!strcmp(paramName, "mode/cont7"))
		sprintf(buf, "%d", m_configData->m_mode7);
	if (!strcmp(paramName, "mode/cont8"))
		sprintf(buf, "%d", m_configData->m_mode8);
	if (!strcmp(paramName, "mode/cont9"))
		sprintf(buf, "%d", m_configData->m_mode9);
	if (!strcmp(paramName, "mode/cont10"))
		sprintf(buf, "%d", m_configData->m_mode10);

	if (!strcmp(paramName, "loot/worms"))
		sprintf(buf, "%d", m_configData->m_lootWorms);
	if (!strcmp(paramName, "loot/inDepot"))
		sprintf(buf, "%d", m_configData->m_lootInDepot);
	if (!strcmp(paramName, "loot/gp"))
		sprintf(buf, "%d", m_configData->m_lootGp);
	if (!strcmp(paramName, "loot/food"))
		sprintf(buf, "%d", m_configData->m_lootFood);
	if (!strcmp(paramName, "loot/custom"))
		sprintf(buf, "%d", m_configData->m_lootCustom);
	if (!strcmp(paramName, "other/autoOpen"))
		sprintf(buf, "%d", m_configData->m_autoOpen);
	if (!strcmp(paramName, "loot/eatFromCorpse"))
		sprintf(buf, "%d", m_configData->m_eatFromCorpse);
