...
int depotDepositTakeFromChest(int objectId,int srcContNr,int srcPos,int qtyToPickup,int depotContNr,int depotContNr2) {
	if(globalAutoAttackStateDepot==CToolAutoAttackStateDepot_noSpace) return 0;
	...
	for (int contNr=0;contNr<memConstData.m_memMaxContainers && qtyToPickup;contNr++) {
...
int AttackCreature(CConfigData *config,int id){
	...
	if (attackedCh){
		if (oldId!=id){
			...
			Sleep(200);
			sender.attack(id);
			if(id) lastSendAttackTm = time(NULL);
...
DWORD WINAPI toolThreadProc( LPVOID lpParam ) {
	...
	if (config->lootFood) {
		lootFromFloorArr.Append(*itemProxy.getFoodIdArrayPtr());
	}
	if (config->lootGp) {
		lootFromFloorArr.Add(itemProxy.getValueForConst("GP"));
	}
	if (config->lootWorms) {
		lootFromFloorArr.Add(itemProxy.getValueForConst("worms"));
	}
	if (config->lootCustom) {
		lootFromFloorArr.Append(*itemProxy.getLootItemIdArrayPtr());
	}
...
//				}else{
				if(1){
					if (config->lootWhileKill){
						...
						if (!lootThreadId){
							if (config->debug) registerDebug("Running lootThreadProc");
							lootThreadHandle = ::CreateThread(NULL,0,lootThreadProc,config,0,&lootThreadId);
						}
						if(!queueThreadId){
							if (config->debug) registerDebug("Running queueThreadProc");
...
void CMod_cavebotApp::loadConfigParam(char *paramName,char *paramValue) {
	...
	if (!strcmp(paramName,"depot/depotDropInsteadOfDepositon")) m_configData->depotDropInsteadOfDeposit=atoi(paramValue);
...
char *CMod_cavebotApp::saveConfigParam(char *paramName) {
	...
	if (!strcmp(paramName,"depot/depotDropInsteadOfDepositon")) sprintf(buf,"%d",m_configData->depotDropInsteadOfDeposit);
