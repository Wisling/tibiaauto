...
int Player_Vocation2VocID(char* resVoc)
{
	...
	if (!strcmp(resVoc, "no vocation"))
		voc = VOCID_NONE;
	else if (!strcmp(resVoc, "knight"))
		voc = VOCID_KNIGHT;
	else if (!strcmp(resVoc, "elite knight"))
		voc = VOCID_KNIGHT | VOCID_PREMIUM;
	else if (!strcmp(resVoc, "paladin"))
		voc = VOCID_PALLADIN;
	else if (!strcmp(resVoc, "royal paladin"))
		voc = VOCID_PALLADIN | VOCID_PREMIUM;
	else if (!strcmp(resVoc, "druid"))
		voc = VOCID_DRUID;
	else if (!strcmp(resVoc, "elder druid"))
		voc = VOCID_DRUID | VOCID_PREMIUM;
	else if (!strcmp(resVoc, "sorcerer"))
		voc = VOCID_SORCERER;
	else if (!strcmp(resVoc, "master sorcerer"))
		voc = VOCID_SORCERER | VOCID_PREMIUM;
...
int CMod_creatureinfoApp::validateConfig(int showAlerts)
{
	...
	{
		if (showAlerts)
			AfxMessageBox("RangeXY must be beween 0 and 100!");
		return 0;
	}
	if (m_configData->rangeZ < 0 || m_configData->rangeZ > 15)
	{
		if (showAlerts)
			AfxMessageBox("RangeZ must be beween 0 and Z!");
		return 0;
	}
	return 1;
}
...
void CMod_creatureinfoApp::loadConfigParam(char *paramName, char *paramValue)
{
	...
	if (!strcmp(paramName, "player/first"))
		lstrcpyn(m_configData->player1, paramValue, MAX_LINE_LEN);
	if (!strcmp(paramName, "player/second"))
		lstrcpyn(m_configData->player2, paramValue, MAX_LINE_LEN);
	if (!strcmp(paramName, "monster/active"))
		m_configData->monster = atoi(paramValue);
	if (!strcmp(paramName, "monster/first"))
		lstrcpyn(m_configData->monster1, paramValue, MAX_LINE_LEN);
	if (!strcmp(paramName, "monster/second"))
		lstrcpyn(m_configData->monster2, paramValue, MAX_LINE_LEN);
	if (!strcmp(paramName, "monster/uniqueMonsterNames"))
		m_configData->uniqueMonsterNames = atoi(paramValue);
	if (!strcmp(paramName, "self/active"))
		m_configData->self = atoi(paramValue);
	if (!strcmp(paramName, "self/first"))
		lstrcpyn(m_configData->self1, paramValue, MAX_LINE_LEN);
	if (!strcmp(paramName, "self/second"))
		lstrcpyn(m_configData->self2, paramValue, MAX_LINE_LEN);
	if (!strcmp(paramName, "self/window"))
		lstrcpyn(m_configData->selfWindow, paramValue, MAX_LINE_LEN);
	if (!strcmp(paramName, "self/tray"))
		lstrcpyn(m_configData->selfTray, paramValue, MAX_LINE_LEN);
	//LOCKED
	//if (!strcmp(paramName,"allFloorInfo"))		m_configData->allFloorInfo = atoi(paramValue);
	if (!strcmp(paramName, "addRequest"))
		m_configData->addRequest = atoi(paramValue);
	if (!strcmp(paramName, "area/collectStats"))
		m_configData->collectStats = 1;
	if (!strcmp(paramName, "area/rangeXY"))
		m_configData->rangeXY = atoi(paramValue);
	if (!strcmp(paramName, "area/rangeZ"))
...
char *CMod_creatureinfoApp::saveConfigParam(char *paramName)
{
	...
	if (!strcmp(paramName, "player/active"))
		sprintf(buf, "%d", m_configData->player);
	if (!strcmp(paramName, "player/first"))
		lstrcpyn(buf, m_configData->player1, MAX_LINE_LEN);
	if (!strcmp(paramName, "player/second"))
		lstrcpyn(buf, m_configData->player2, MAX_LINE_LEN);
	if (!strcmp(paramName, "monster/active"))
		sprintf(buf, "%d", m_configData->monster);
	if (!strcmp(paramName, "monster/first"))
		lstrcpyn(buf, m_configData->monster1, MAX_LINE_LEN);
	if (!strcmp(paramName, "monster/second"))
		lstrcpyn(buf, m_configData->monster2, MAX_LINE_LEN);
	if (!strcmp(paramName, "monster/uniqueMonsterNames"))
		sprintf(buf, "%d", m_configData->uniqueMonsterNames);
	if (!strcmp(paramName, "self/active"))
		sprintf(buf, "%d", m_configData->self);
	if (!strcmp(paramName, "self/first"))
		lstrcpyn(buf, m_configData->self1, MAX_LINE_LEN);
	if (!strcmp(paramName, "self/second"))
		lstrcpyn(buf, m_configData->self2, MAX_LINE_LEN);
	if (!strcmp(paramName, "self/window"))
		lstrcpyn(buf, m_configData->selfWindow, MAX_LINE_LEN);
	if (!strcmp(paramName, "self/tray"))
		lstrcpyn(buf, m_configData->selfTray, MAX_LINE_LEN);
	if (!strcmp(paramName, "allFloorInfo"))
		sprintf(buf, "%d", m_configData->allFloorInfo);
	if (!strcmp(paramName, "addRequest"))
		sprintf(buf, "%d", m_configData->addRequest);
	if (!strcmp(paramName, "area/collectStats"))
		sprintf(buf, "%d", m_configData->collectStats);
	if (!strcmp(paramName, "area/rangeXY"))
		sprintf(buf, "%d", m_configData->rangeXY);
	if (!strcmp(paramName, "area/rangeZ"))
		sprintf(buf, "%d", m_configData->rangeZ);
