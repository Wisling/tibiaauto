...
static void DeleteGUITreeItem(CTreeCtrl* treeCtrl, HTREEITEM treeItem, HTREEITEM rootItem /*=0*/)
{
	...
	if (treeItem != TVI_ROOT)
	{
		...
		if (treeItem == rootItem)
			treeCtrl->DeleteItem(rootItem);                //delete everything at the end
	}
...
void CToolItemConfig::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (Dragging)
	{
		...
		if (htDest.hItem && isLegalMove)
		{
			...
			if (dest.hParent == NULL)
				dest.hParent = TVI_ROOT;
			dest.hInsertAfter = insertAfter ? htDest.hItem : m_itemsTree.GetPrevSiblingItem(htDest.hItem);
			if (dest.hInsertAfter == NULL)
				dest.hInsertAfter = TVI_FIRST;

			if (dest.hInsertAfter == itemOrigin)
				return;
		...
		if (pTVKeyDown->wVKey == 'V')
		{
			if (actionIndicator)
			{
				...
				if (item == NULL)
					...
					dest.hParent = m_itemsTree.GetParentItem(item);
				if (dest.hParent == NULL)
					dest.hParent = TVI_ROOT;
				...
				if (dest.hInsertAfter == actionItem)
					return;
	...
	while (child != NULL)
	{
		...
		else if (!treeCtrl->ItemHasChildren(child) && treeCtrl->GetItemData(child) != 0)
		{
			...
			if (check == 2)
			{
				...
				CGUITreeItemData* itemData = (CGUITreeItemData*)treeCtrl->GetItemData(child);
				if (itemData)
					treeCtrl->SetItemData(item, (long)new CGUITreeItemData(itemData->id, itemData->type));
			}
		}
...
void CToolItemConfig::OnSelectedToBranch()
{
	...
	if (parentItem == NULL)
		parentItem = TVI_ROOT;
	if (parentItem != TVI_ROOT && m_itemsTree.GetItemData(parentItem) != 0)
		parentItem = m_itemsTree.GetParentItem(parentItem);
	if (parentItem == NULL)
		parentItem = TVI_ROOT;
