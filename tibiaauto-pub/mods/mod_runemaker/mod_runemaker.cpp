...
int CMod_runemakerApp::validateConfig(int showAlerts)
{
	...
	{
		if (showAlerts)
			AfxMessageBox("Soul points must be >=0!");
		return 0;
	}
	if (strlen(m_configData->spell) == 0)
	{
		if (showAlerts)
			AfxMessageBox("Please enter rune spell!");
		return 0;
	}
...
void CMod_runemakerApp::loadConfigParam(char *paramName, char *paramValue)
{
	...
	if (!strcmp(paramName, "manaLimit"))
		m_configData->manaLimit = atoi(paramValue);
	if (!strcmp(paramName, "soulPoints"))
		m_configData->soulPoints = atoi(paramValue);
	if (!strcmp(paramName, "premium"))
		m_configData->premium = atoi(paramValue);
	if (!strcmp(paramName, "maxUse"))
		m_configData->maxUse = atoi(paramValue);
	if (!strcmp(paramName, "randomCast"))
		m_configData->randomCast = atoi(paramValue);
	if (!strcmp(paramName, "useSpear"))
		m_configData->useSpear = atoi(paramValue);
...
char *CMod_runemakerApp::saveConfigParam(char *paramName)
{
	...
	if (!strcmp(paramName, "spell"))
		sprintf(buf, "%s", m_configData->spell);
	if (!strcmp(paramName, "mana"))
		sprintf(buf, "%d", m_configData->mana);
	if (!strcmp(paramName, "manaLimit"))
		sprintf(buf, "%d", m_configData->manaLimit);
	if (!strcmp(paramName, "soulPoints"))
		sprintf(buf, "%d", m_configData->soulPoints);
	if (!strcmp(paramName, "premium"))
		sprintf(buf, "%d", m_configData->premium);
	if (!strcmp(paramName, "maxUse"))
		sprintf(buf, "%d", m_configData->maxUse);
	if (!strcmp(paramName, "randomCast"))
		sprintf(buf, "%d", m_configData->randomCast);
	if (!strcmp(paramName, "useSpear"))
		sprintf(buf, "%d", m_configData->useSpear);
